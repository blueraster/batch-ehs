...
#   b - Aggregates each loss year raster by a scale factor of 80, which produces an aggregated raster of 500 rows and columns 
#       (40000 native resolution Hansen tile columns and rows / 80 aggregation factor = 500 rows and columns), 
#       and aggregating by counting the number of original resolution cells in each aggregated cell.
...
loss_tiles_path = r'D:\Emerging_hotspots\GFW2015'
tree_cover_density_path = r'D:\Emerging_hotspots\TCD'
intermediate_files = r'C:\Users\deitelberg\Documents\ArcGIS\Projects\Emerging Hotspots\Brazil_2015_cellFactor80.gdb'
global_points_name = 'Brazil_2015_cellFactor80'
        ...
        for tcd in tcd_list:
            if look_for in tcd:
                ...
                try:
                    ...
                    print('     BUSTED LOSS TILE!!!  {}'.format(tile))
                    continue
...
for short_year in value_years:
    ...
    for raster in mskd_tile_list:
        ...
        start = timer()
        arcpy.RasterToPoint_conversion(aggregate_raster, out_points, "Value")
        print("         Elapsed time: {}".format( round(elapsed_time(start, timer()), 2)))
        write_to_log(log, "         Elapsed time: {}".format( round(elapsed_time(start, timer()), 2)))
